...
class GameViewController: UIViewController {
    ...
    @IBOutlet weak var stackView_hardMode: UIStackView!
    ...
    override func viewDidLoad() {
        ...
        case "EasyMode":
            ...
            stackView_hardMode.isHidden = true
            game.initGame(cards: game.game_images_easy)
            
        case "HardMode":
            
            stackView_hardMode.isHidden = false
            game.initGame(cards: game.game_images_hard)
    ...
    func gameOver(card: UIButton){
        if(checkWinPairs == 8 && receivedData == "EasyMode")
        {
            ...
            game.setSuccessCounter(successCounter: 0)
        }else if(checkWinPairs == 10 && receivedData == "HardMode"){
            game.setTimer(on: false , timerLabel: game_timer)
            let scoreNewIndex = checkifHighScore(score: game.getScore() - finalScore)
            if scoreNewIndex {
                isPermission = findLocation()
